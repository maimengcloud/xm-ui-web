...
<span> 
    ...
    <el-select :size="size" :multiple="multiple" :placeholder="placeholder" :disabled="disabled" :style="{width:width?width+'':'auto'}" class="my-select" ref="selectRef" v-model="myVal" @change="onChange" :clearable="clearable" @visible-change="$emit('visible-change',$event)" @focus="$emit('focus',$event)" @blur="$emit('blur',$event)" @clear="$emit('blur',$event)" @click="$emit('click',$event)">
        <slot> 
            <el-option disabled value="" style="margin-bottom:5px;">
              <el-row><el-button @click.stop="$refs['deptDialog'].open()" icon="el-icon-setting">部门树</el-button></el-row>
            </el-option>
      ...
      <span class="field-oper" :class="{disabled:disabled===true,enabled:disabled!==true,hidden:true}">
        <slot name="oper">
          <el-select :size="size" :style="{width:width?width+'':'auto'}" class="my-select" ref="selectRef" v-model="myVal" @change="onChange" :clearable="clearable" @visible-change="$emit('visible-change',$event)" @focus="$emit('focus',$event)" @blur="$emit('blur',$event)" @clear="$emit('blur',$event)" @click="$emit('click',$event)">
              <slot>
                  <el-option disabled value="" style="margin-bottom:5px;">
                    <el-row><el-button @click.stop="$refs['deptDialog'].open()">部门树</el-button></el-row>
                  </el-option> 
                  <el-option class="avatar-container" v-for="(option,index) in optionsCpd" :key="index" :value="option[props['id']]" :label="option[props['name']]">  
  ...
  <mdp-dialog ref="deptDialog" title="选择部门" width="80%">
      <template v-slot="{visible,data,dialog}">
        <dept-tree show-confirm :visible="visible" showFilter :multiple="multiple"  show-checkbox :expand-on-click-node="false" @confirm="onDeptSelect"/>
      </template>
    </mdp-dialog>
  </span>
</template> 
...
import DeptTree from './DeptTree';//下拉框数据查询
var deptCacheList=[]
export default {
  components: { DeptTree },
  name: 'mdp-select-dept',
  ...
  data(){
    return {
      localStorageKey:'mdp-common-dept-list',
    }
  },
  methods:{
    
    initItemOptions(){  
      if(deptCacheList && deptCacheList.length>0){
        this.item={itemType:'4',options:deptCacheList}
      }else{
        var deptsStr=localStorage.getItem(this.localStorageKey)
        if(deptsStr){
          var depts=JSON.parse(deptsStr)
          this.item={itemType:'4',options:depts}
          deptCacheList=depts
        }else{
    ...
    onDeptSelect(depts){   
      var props=this.props||{id:'deptid',name:'deptName'}
      if(this.multiple){
         depts.forEach(k=>{
          if(!this.optionsCpd.some(m=>m[props.id]==k[props.id])){
            this.item.options.unshift(k) 
          }
          if(!deptCacheList.some(m=>m[props.id]==k[props.id])){
            deptCacheList.unshift(k) 
          }
         })
         this.myVal=depts.map(k=>k[props['id']])
         this.onChange(this.myVal)
      }else{ 
        if(!this.optionsCpd.some(m=>m[props.id]==depts[props.id])){
          this.item.options.unshift(depts) 
        }
        if(!deptCacheList.some(m=>m[props.id]==depts[props.id])){
          deptCacheList.unshift(depts) 
        }
        this.myVal=depts[props['id']] 
        this.onChange(this.myVal)
      }
      this.onClose();
    ...
    onClose(){
      localStorage.setItem(this.localStorageKey,JSON.stringify(deptCacheList));
    }
  },
  mounted(){
      this.initData();
