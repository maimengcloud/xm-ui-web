	...
	<section>
			...
			<div class="custom-tree-node" slot-scope="{ node, data}">   
				<el-popover
					...
					<el-button icon="el-icon-delete" circle type="danger" v-if=" !(!data.id || data.id=='C0'||data.id=='0')" @click.prevent.stop="deleteNode(data,node)" title="删除该分类"> </el-button>
 					<span slot="reference">{{data.categoryName}}</span>
				</el-popover>
			</div>
		</el-tree>
		<el-dialog
		...
		<el-dialog
			...
			width="60%" append-to-body> 
				...
				<el-row slot="footer"> 
					<el-button @click="addVisible = false">关 闭</el-button>
					<el-button type="primary" v-loading="addLoading"  @click="editImageCategorySubmit">确 定</el-button>
				</el-row>
		</el-dialog>
		</el-row>
	</section>
		...
		methods: { 
				...
				this.$refs.addImageCategory.validate((valid) => {
					if (valid) {
						this.$confirm('确认提交吗？', '提示', {}).then(() => {
							...
							this.$mdp.addImageCategory(this.addImageCategory).then(res=>{
				...
				this.$refs.editImageCategory.validate((valid) => {
					if (valid) {
						this.$confirm('确认提交吗？', '提示', {}).then(() => {
							...
							this.$mdp.editImageCategory({id:this.editImageCategory.id,categoryName:this.editImageCategory.categoryName,branchId:this.editImageCategory.branchId}).then(res=>{
								//console.log("res--"+JSON.stringify(res));
		    ...
		    deleteNode(data, node, comp) {   
						...
						this.$confirm('确认删除吗？', '提示', {}).then(() => { 
							this.$mdp.delImageCategory(params).then(res=>{
								//console.log("res--"+JSON.stringify(res));
								if(res.data.tips.isOk){
									this.editVisible=false;
			...
			getTreeData(refresh) {
				...
				this.$mdp.listImageCategory(params).then((res) => {
