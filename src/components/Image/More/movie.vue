<template>
	...
	<div class="SpeListOverFlow">
		    ...
		    <el-card :body-style="{ padding: '0px' }">
		      <div class="avatar-uploader" @click="selectedImage(index)">
		      	<div style="height: 178px;width: 186px;display: flex;">
					...
					<video v-if="getUrl(o)" :src="converUrl(o)"  class="avatar" autoplay loop muted ></video>
		      ...
		      <span class="row-span" style="justify-content:center;align-items:center;display:flex;">
				...
				<el-button type="text" class="button" @click="previewImg(index)">播放</el-button>
		      </span>
		    </el-card>
		</el-col>
		...
		<el-dialog title="选择视频" :visible.sync="addFormVisible"  width="70%" :close-on-click-modal="false" append-to-body>
		...
		<el-dialog :visible.sync="previewVisible" append-to-body>
			  ...
			  <video  :src="imageUrl" width="100%" height="480" controls ></video>
		</el-dialog>
		</div>
	</section>
</template>
...
<script>
	...
	import UploadImage from '../UploadMovie';
	...
	export default {
		...
		watch: {
			...
			'imageLists':function(val){
				this.$emit('input',val);
				console.log("视频的this.imageLists------------------------------------");
		...
		methods: {
			selectedImage(index){//点击选择了已经有图片的框,执行
			...
			previewImg(index){
				...
				console.log("视频路径为"+this.imageUrl)
				this.previewVisible=true; 
			},
			addImg(){ //点击+按钮触发
				if(parseInt(this.limit)<=this.imageLists.length){
					this.$message({ message: "只能上传一个视频", type: 'error' });
					return;
				}
				this.addFormVisible = true;
				this.opflag='add';
