<template>
	<section  class="page-container padding">
		...
		<!--编辑界面 MenuModuleBranch 管理端机构表（机构下面若干部门）--> 
		...
		<el-dialog width="60%" top="20px" :visible.sync="moduleSelectVisible" append-to-body>
			<menu-module-select :single="true" @row-click="onModuleRowClick"></menu-module-select>
		</el-dialog>
	</section>
</template>
...
<script>
	...
	import config from "@/common/config"; //全局公共库import
 	import { initDicts, addMenuModuleBranch,editMenuModuleBranch } from '@/api/mdp/menu/menuModuleBranch';
	import { mapGetters } from 'vuex'
	import BranchSelect from "@/views/mdp/sys/branch/BranchSelect"
	import MenuModuleSelect from "@/views/mdp/menu/menuModule/ModuleSelect"
	export default {
	    name:'menuModuleBranchEdit',
	    components: {
			BranchSelect,MenuModuleSelect,
        },
		computed: {
		    ...mapGetters([ 'userInfo'  ]),

		},
		props:['menuModuleBranch','visible','opType'],

		watch: {
	      'menuModuleBranch':function( menuModuleBranch ) {
	        if(menuModuleBranch){
	            this.editForm = {...menuModuleBranch};
	        }
		...
		methods: {
			...
			//新增、编辑提交MenuModuleBranch 管理端机构表（机构下面若干部门）父组件监听@submit="afterEditSubmit"
			saveSubmit: function () {
				this.$refs.editFormRef.validate((valid) => {
					if (valid) {
						this.$confirm('确认提交吗？', '提示', {}).then(() => { 
							...
							var func=addMenuModuleBranch
							if(this.currOpType=='edit'){
							    func=editMenuModuleBranch
							}
			...
			initData: function(){
			    ...
			    if(this.menuModuleBranch){
                    this.editForm = Object.assign({},this.menuModuleBranch);
                }
