		...
		methods: {
			...
			showSendSms:function(){
				if(this.sels.length>1){
 					this.$notify({position:'bottom-left',showClose:true,message: '一次只能给一个人发短信', type: 'error' });
				}else if(this.sels.length<1){
					this.$notify({position:'bottom-left',showClose:true,message: '请选择一条任务', type: 'error' });
				}else{
			...
			showWeixinTask:function(){
				if(this.sels.length>1){
					 this.$notify({position:'bottom-left',showClose:true,message: '一次只能给一个人发信', type: 'error' });
					...
					createShortToken({userid:task.assignee,branchId:this.userInfo.branchId}).then(res=>{
						if(res.data.tips.isOk){
							...
							this.$copyText(this.weixinContent).then(e=>{ 
								this.$notify({position:'bottom-left',showClose:true,message: '已拷贝内容，您可黏贴到微信中', type: 'success' });
							});
						}
					});
