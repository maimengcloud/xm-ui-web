          ...
          <el-row class="page-main">
            ...
            >
              ...
              <el-table-column sortable prop="tagNames" label="标签" min-width="80"  show-overflow-tooltip>
                <template slot-scope="scope">
                  <el-tag
                    v-for="(tagName,index) in (scope.row.tagNames?scope.row.tagNames.split(','):[])"
                    :key="index"
                  >{{tagName}}</el-tag>
                </template>
              </el-table-column>
            ...
            >
              <procdef-parames-set
                :procdef="addForm"
                @cancel="procdefParamesVisible=false"
                @submit="procdefParamesVisible=false"
              ></procdef-parames-set>
            </el-dialog>
            <el-dialog
              title="业务关联设置"
              :visible.sync="bizModelVisible"
              width="80%"
              :close-on-click-modal="false"
              append-to-body
            >
              <biz-model-mng
                :model-key="addForm.key"
                :model-name="addForm.name"
                :visible="bizModelVisible"
                @cancel="bizModelVisible=false"
                @submit="bizModelVisible=false"
              ></biz-model-mng>
            </el-dialog>
...
export default {
  ...
  methods: {
    ...
    showProcdefParamesSet: function(row, index) {
      this.addForm = row;
      this.procdefParamesVisible = true;
    },
    formatterSuspensionState(row, column, cellValue) {
      return cellValue == 1 ? "激活的" : "挂起的";
    },
    showBizModelDialog: function(row, index) {
      this.addForm = row;
      this.bizModelVisible = true;
    },
    handleDownload() {
  ...
  components: {
    //在下面添加其它组件
    "procdef-parames-set": ProcdefParamesSet,
    sticky: Sticky,
    CategoryTree,
    TagMng,
    BizModelMng
