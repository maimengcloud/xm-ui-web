	...
	<section class="border padding"> 
				...
				<el-row>
					...
					<el-table :ref="refId+'Table'" :height="100" v-adaptive="{bottomOffset: bottomOffset}" :data="tableDatas" @sort-change="sortChange" highlight-current-row v-loading="load.list" border @selection-change="selsChange" @row-click="rowClick" style="width: 100%;">
						...
						<el-table-column prop="username"  label="用户名称" min-width="120" show-overflow-tooltip fixed="left" col-type="String" v-if="showCol('username')">
						...
						<el-table-column label="操作" width="180" fixed="right">
							...
							<template scope="scope"> 
								<el-button :disabled="disabledJudge('delBtn') || !checkBtnQx('delBtn',menuDefId) " type="text" @click="handleDel(scope.row,scope.$index)" icon="el-icon-delete"  title="同意注销">同意注销</el-button>
							</template>
						</el-table-column>
					</el-table>
				</el-row>
...
export default {
    ...
    data() {
        return {
            ...
            currOpType:'unreg',//表格 mng-综合管理具有最大权限，所有按钮可动、detail-只看不能操作
            filters:{//查询参数
