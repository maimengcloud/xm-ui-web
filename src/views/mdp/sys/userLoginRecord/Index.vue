<template>
			...
			<el-table :ref="refId+'Table'" :height="100" v-adaptive="{bottomOffset: bottomOffset}" :data="tableDatas" @sort-change="sortChange" highlight-current-row v-loading="load.list" border @selection-change="selsChange" @row-click="rowClick" style="width: 100%;">
				...
				<el-table-column label="操作" :width="currOpType=='mng'?200:180" fixed="right">
				    ...
				    <template scope="scope" v-if="currOpType=='mng'"  >
				        <el-button :disabled="disabledJudge('editBtn') || !checkBtnQx('editBtn',menuDefId) " type="primary" @click="openForm({parentOpType:currOpType,subOpType:'detail',formData:scope.row,title:'修改'})" icon="el-icon-edit" title="修改一条数据"/>
 				    </template>
		...
		<el-row>
			<!--新增修改明细 UserLoginRecord 用户登录信息登记界面-->
            <mdp-dialog :ref="refId+'FormDialog'">
                <template v-slot="{visible,data,dialog}">
                     <user-login-record-form :ref="refId+'Form'" :visible="visible" :parent-op-type="currOpType" :sub-op-type="data.subOpType" :form-data="data.formData" @close="onFormClose" @submit="afterFormSubmit" @fields-change="afterEditSomeFields"/>
                </template>
            </mdp-dialog>
 	    </el-row>
	</section>
...
import  UserLoginRecordForm from './Form';//新增修改明细界面
...
export default {
    ...
    data() {
        return {
            ...
            //增删改查(含批量)接口
            apis:{
                list: UserLoginRecordApi.listUserLoginRecord,
                add: UserLoginRecordApi.addUserLoginRecord,
                del: UserLoginRecordApi.delUserLoginRecord,
                edit: UserLoginRecordApi.editUserLoginRecord,
                editSomeFields: UserLoginRecordApi.editSomeFieldsUserLoginRecord,
                batchAdd: UserLoginRecordApi.batchAddUserLoginRecord,
                batchDel: UserLoginRecordApi.batchDelUserLoginRecord,
                batchEdit: UserLoginRecordApi.batchEditUserLoginRecord,
            }
        }
    },
    methods: {
