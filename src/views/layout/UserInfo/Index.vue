<template>
  <div>
    <el-dropdown class="avatar-container right-menu-item hidden-sm-and-down" trigger="click"  @command="handleCommand" @click.native="onDropdownClick">
        ...
        <el-dialog
          title="请选择一个账户进行登录"
          :visible.sync="switchUserVisible"
          width="800" append-to-body> 
          <el-table :data="phonenoUsers"> 
            ...
            <el-table-column prop="defLogin" label="默认">
                <template slot-scope="scope">
                    <el-checkbox v-model="scope.row.defLogin" true-label="1" false-label="0" @change="(e)=>onDefLoginChange(scope.row,e)"></el-checkbox>
                </template> 
            </el-table-column> 
...
import { setToken,removeCacheUserInfo,getToken} from '@/utils/auth'
import { mapGetters } from 'vuex'
import dayjs from 'dayjs' 
import md5 from 'js-md5';
import {  queryMyUsers,switchUser,setDefLogin } from '@/api/login';
...
export default {
    ...
    methods: {
        onDefLoginChange(user,defLogin){
            setDefLogin(user.userid,defLogin).then(res=>{
                var tips=res.data.tips
                if(tips.isOk){
                    this.$notify.success(tips.msg)
                }else{
        ...
        getBranchInterestsDetail(){
            this.$mdp.getBranchInterestsDetail().then(res=>this.branchUsersCount=res.data.data||this.branchUsersCount)
        },
        getMyRoleNames(){
        ...
        handleCommand(command){
            if(process.env.CONTEXT==this.$mdp.getSysContext()){
                ...
                if(command=='branchSet'){
                    this.$router.push({path:'/mdp/sys/branch/set'})
                }
                ...
                if(command=='branchSet'){ 
                    window.open(prefixUrl+'mdp/sys/branch/set') 
                    NProgress.done()
                }
