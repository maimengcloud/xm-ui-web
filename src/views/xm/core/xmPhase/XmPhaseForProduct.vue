	...
	<section class="padding">
		<el-row v-show="batchEditVisible==false"> 
			<span  v-if="batchEditVisible!=true"   >
				...
				<el-button  class="hidden-md-and-down"  @click="loadMenusToXmPhase(sels)" v-loading="load.edit" icon="el-icon-s-data">由需求汇总进度数据</el-button> 
			...
			<el-popover
					...
					<el-row> 
						...
						<el-col  :span="24"  style="padding-top:5px;">
							<el-button  type="warning" @click="loadMenusToXmPhase(sels)" v-loading="load.edit" icon="el-icon-s-data">由需求汇总进度数据</el-button> 
						</el-col>    
					</el-row>
								...
								<el-dropdown @command="handleCommand" :hide-on-click="false"> 
									...
									<el-dropdown-menu slot="dropdown">
										...
										<el-dropdown-item icon="el-icon-search"   :command="{type:'showLog',row:scope.row}">日志</el-dropdown-item> 									
  										<el-dropdown-item icon="el-icon-success"   :command="{type:'loadMenusToXmPhase',row:scope.row}">从需求中汇总进度</el-dropdown-item> 
		...
		<el-row v-if="batchEditVisible==true">
			<xm-phase-batch :xm-product="xmProduct" @back="batchEditBack"></xm-phase-batch>
		</el-row>
	</section>
</template>
      ...
      handleGanttData(list, keys) {
        ...
        return row;
			...
			handleCommand(command) { 
				...
				}  else if(command.type=='loadMenusToXmPhase'){
					this.loadMenusToXmPhase([command.data]);
				} else if(command.type=='showTaskForBatchRelTasksWithPhase'){
					this.taskVisible=true
				}
			},
			//从任务中汇总进度/实际费用等数据
			loadMenusToXmPhase:function(phases){ 
				var params={
				...
				if(phases && phases.length>0){
				 ...
				 this.load.edit=true;
				loadMenusToXmPhase(params).then(res=>{
