<template>
	<section>
		...
		<!--编辑界面 XmBranchTaskTypeState 按机构编号任务类型汇总--> 
	...
	export default { 
		...
		watch: {
	      'xmBranchTaskTypeState':function( xmBranchTaskTypeState ) {
	        this.editForm = xmBranchTaskTypeState;
	      },
		...
		methods: {
			...
			//编辑提交XmBranchTaskTypeState 按机构编号任务类型汇总父组件监听@submit="afterEditSubmit"
			editSubmit: function () {
				this.$refs.editForm.validate((valid) => {
					if (valid) {
						this.$confirm('确认提交吗？', '提示', {}).then(() => { 
							...
							editXmBranchTaskTypeState(params).then((res) => {
		...
		mounted() {
			this.editForm=Object.assign(this.editForm, this.xmBranchTaskTypeState);  
		}
	}
