<template>
	<section class="page-container  padding border">
		<el-row> 
		<!--编辑界面 XmMenuState 功能状态表,无需前端维护，所有数据由汇总统计得出--> 
...
<script>
	...
	export default { 
		...
		watch: {
	      'xmMenuState':function( xmMenuState ) {
	        this.editForm = xmMenuState;
	      },
		...
		methods: {
			...
			//编辑提交XmMenuState 功能状态表,无需前端维护，所有数据由汇总统计得出父组件监听@submit="afterEditSubmit"
			editSubmit: function () {
				this.$refs.editForm.validate((valid) => {
					if (valid) {
						this.$confirm('确认提交吗？', '提示', {}).then(() => { 
							...
							editXmMenuState(params).then((res) => {
		...
		components: {  
		    //在下面添加其它组件 'xm-menu-state-edit':XmMenuStateEdit
		},
		mounted() {
			this.editForm=Object.assign(this.editForm, this.xmMenuState);  
		}
	}
